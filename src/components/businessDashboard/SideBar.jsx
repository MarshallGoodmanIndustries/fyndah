...
const SideBar = ({ handleToggle }) => {
  ...
  const { authToken, setAuthToken, setUserData, setBusinessId } =
  ...
  const handleLogOut = async () => {
    try {
      ...
      Swal.fire({
        ...
        title: "Oops...",
        text: "Logout Failed!",
        footer: `<a href="#">Could not log out. Please try again later. ${
    ...
    <div className="px-[1rem] text-white font-inter py-[1rem]">
      ...
      <Link to="leads">
        <div
          ...
          className={`flex cursor-pointer mb-1 hover:bg-white rounded-[4px] hover:text-textDark px-[1rem] py-[0.5rem] items-center justify-start gap-4 ${
            isItemActive(2) ? "bg-white text-textDark" : "text-white"
          } `}
        >
          <span>
